...
int main()
{
	...
	while (true)
	{
		...
		std::cout << "10. Add entry after existing entry\n";
		std::cout << "0. Exit\n\n";
		std::cout << "Input: ";
		menu = input_int(0, 10);
		switch (menu)
		...
		case 10:
		{
			...
			std::cout << "Enter name of entry to add after: ";
			std::string name = input_string_cap();
			int pos = staff.findpos(name);
			if (pos == -1)
			{
...
//prompts user to input name and add entry after entry with that name
void database::addentryafter(int pos)
{
	entry temp;
	++pos;
	std::cout << "Enter a name: ";
